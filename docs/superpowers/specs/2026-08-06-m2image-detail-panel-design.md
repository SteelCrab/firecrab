...
다른 alias 상세를 보는 동안 A alias의 **설치** 또는 **굽기** 작업이
진행 중이면, 그 로그/스텝퍼/라이브 콘솔은 A를 다시 선택하기 전까지
화면에 보이지 않는다(작업 자체는 계속 폴링되며 끊기지 않는다 — 굽기의
경우 라이브 콘솔 WebSocket만 언마운트되고, 재연결 시 서버가 들고 있는
최근 출력 버퍼가 다시 재생되므로 스크롤백이 완전히 유실되지는 않는다).
MicroNetworks/MicroStorages도 선택 해제 시 상세를 숨기는 동일 패턴이라
이 코드베이스 안에서 일관된 선택이다 — 논의 후 승인됨.
