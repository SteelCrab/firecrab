...
- [동시 부팅 시 멀쩡한 VM이 SIGKILL됨](../../50-bugs/vm-killed-mid-boot-under-concurrent-load.md)
      — broadcast의 `Lagged`(컨슈머 지연)를 `Closed`로 오인
- [DHCP가 거의 항상 실패](../../50-bugs/dhcp-never-reaches-guest.md)
      — 원인 3건: dnsmasq 설정/hosts 파일 경로 충돌, `dhcp-hostsfile` 접두어, IP 빠른 재사용
- [Alpine만 `no-ipv4-address`](../../50-bugs/alpine-network-ready-races-dhcpcd.md)
      — OpenRC `after dhcpcd`는 시작 순서만 보장, 완료를 보장하지 않음
- [VM 아웃바운드 타임아웃](../../50-bugs/vm-outbound-forward-blocked-by-ufw.md)
      — 호스트 UFW가 라우팅을 기본 거부. 코드가 아니라 호스트 설정 문제
- [Alpine 공식 커널 부팅 실패](../../50-bugs/alpine-official-kernel-cant-mount-root.md)
...
- 테스트 절차는 [MicroNetwork 테스트 문서](../../40-tests/micro-network.md) 등 `docs/tests/` 참고
