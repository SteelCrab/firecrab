...
- 네트워크 구성: bridge 이름, subnet CIDR, gateway — 지금은 고정값 노출만 되고 편집은 안 됨(IPAM/
  bridge 재구성이 필요한 후속 작업, `docs/task-network-configuration-dashboard.md` 참고)
- host 상태: load average(1분), memory(total/available), disk(total/available), uptime — 2초
  주기로 갱신되는 실시간 스냅샷(`/proc`, `df` 기준)
