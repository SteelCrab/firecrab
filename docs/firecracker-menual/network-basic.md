...
2. `scripts/firecracker-menual/boot-microvm.sh`
...
3. `scripts/firecracker-menual/serial-console.sh`를 실행한다.
4. `scripts/firecracker-menual/boot-microvm.sh`가 `/sys/class/net/tap0`를 감지해 `firecracker-config.json`의 `network-interfaces`에 자동으로 추가한다.
